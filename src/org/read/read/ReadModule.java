...
public class ReadModule {
	public static List<String> readOriginal(String a) {
		String address = a;
	    ...
	    try {
	      ...
	      myReader.close();
			...
			textbook.error("FileNotFoundException - READ MODULE ERROR, CHK ORIGIN FILE/PATH");
//	      e.printStackTrace();
	    }
	    return list;
	  }
		...
		try {
			...
			while (myReader.hasNextLine()) {
				...
				if (data.contains(DNRsign)) { // DNR SIGN
					if (data.contains("startdnrfalse"))
						StartDNR = false;
